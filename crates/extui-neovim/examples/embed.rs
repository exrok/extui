...
//!
//! Used by `tools/compare_embed.py` to diff the rendered grid against a
//! native `nvim` TUI with identical size and input.
...
fn main() -> std::io::Result<()> {
    ...
    loop {
        ...
        let rect = Rect { x: 0, y: 0, w: buf.width(), h: buf.height() };
