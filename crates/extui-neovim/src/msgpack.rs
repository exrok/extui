...
///
/// The options enable `ext_linegrid` and `ext_termcolors`. The former
/// selects the modern line-grid protocol. The latter tells Neovim to
/// report `cterm_fg = cterm_bg = -1` in `default_colors_set`, so the
/// client can inherit the host terminal's own default colors instead of
/// painting Neovim's synthesized RGB fallback.
///
/// Sent as MessagePack-RPC request type 0.
pub fn encode_ui_attach(w: &mut Writer, msgid: u32, width: u16, height: u16) {
...
/// Encodes a `nvim_set_option_value` notification that turns a boolean
/// option off.
///
/// Written right after `nvim_ui_attach` to pin `termguicolors` off, so
/// nvim's highlight resolver runs its cterm path for this UI. Sent as
/// a notification (msg type 2) — we don't care about the result.
pub fn encode_set_option_bool(w: &mut Writer, name: &str, value: bool) {
    w.write_array_header(3);
    w.write_u64(2);
...
mod tests {
    ...
    fn ui_attach_is_a_request() {
        let mut w = Writer::new();
        encode_ui_attach(&mut w, 7, 80, 24);
        let bytes = w.into_bytes();
        let mut r = Reader::new(&bytes);
        ...
        assert_eq!(r.read_map_len().unwrap(), 3);
