...
impl NeovimEmbed {
    ...
    pub fn spawn_with(mut cmd: Command, width: u16, height: u16) -> io::Result<Self> {
        ...
        // Queue `ui_attach` and, right after it,
        // `nvim_set_option_value('termguicolors', false)`. This pins
        // nvim's highlight resolver onto its cterm path for our UI:
        // gui-only colorschemes get auto-quantised to cterm indices by
        // nvim itself (using the same palette the user's 256-colour
        // terminal interprets), and the `cterm_attr` dict in each
        // `hl_attr_define` now reflects the active colorscheme instead
        // of unrelated built-in defaults. We don't need to query
        // anything back — we're forcing the state.
        //
        // The two requests are written together so nvim processes
        // them contiguously after startup, and the handful of
        // `hl_attr_define` events that land between them get
        // superseded by the subsequent `ui_refresh` that
        // `termguicolors` triggers.
        let mut writer = Writer::with_capacity(160);
        mp::encode_ui_attach(&mut writer, UI_ATTACH_MSGID, width, height);
        mp::encode_set_option_bool(&mut writer, "termguicolors", false);
