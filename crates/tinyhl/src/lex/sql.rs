...
fn scan_operator(view: &mut SourceView<'_>, cursor: u32, b0: u8) -> (u16, u32) {
    ...
    let three = match (b0, b1, b2) {
        (b'-', Some(b'>'), Some(b'>')) => Some(kinds::THIN_ARROW),
        (b'#', Some(b'>'), Some(b'>')) => Some(kinds::HASH),
        (b'!', Some(b'~'), Some(b'*')) => Some(kinds::BANG),
        (b'~', Some(b'~'), Some(b'*')) => Some(kinds::TILDE),
        _ => None,
    };
    if let Some(k) = three {
        return (k, cursor + 3);
    ...
    if let Some(b1) = b1 {
        let two = match (b0, b1) {
            ...
            (b':', b':') => Some(kinds::COLON),
            (b'-', b'>') => Some(kinds::THIN_ARROW),
            (b'=', b'>') => Some(kinds::FAT_ARROW),
            (b'#', b'#') => Some(kinds::HASH_HASH),
            (b'#', b'>') | (b'#', b'-') => Some(kinds::HASH),
            (b'@', b'>') | (b'@', b'@') | (b'@', b'?') => Some(kinds::AT),
            (b'<', b'@') => Some(kinds::LT),
            (b'!', b'~') => Some(kinds::BANG),
            (b'~', b'*') | (b'~', b'=') | (b'~', b'~') => Some(kinds::TILDE),
            (b'?', b'&') | (b'?', b'|') | (b'?', b'-') | (b'?', b'#') => Some(kinds::QUESTION),
            _ => None,
        };
        if let Some(k) = two {
            return (k, cursor + 2);
